...
start.addEventListener("click", () => {
    ...
    let interval = 3000;
    if (difficulty.value === "medium") interval = 2000;
    if (difficulty.value === "hard") interval = 1000;

    bumpInterval = setInterval(randomBump, interval);
    timerId = setInterval(countDown, 1000);
});

id.classList.add("hit");
setTimeout(() => id.classList.remove("hit"), 300);
